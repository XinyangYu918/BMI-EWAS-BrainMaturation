...
sd <- read.csv("CAT12_results/ROI_aparc_DK40_sqrtdepth.csv")
sd <- sd[,-c(3,4,11,12)]
...
for (c in 3:70)
{
  ...
  # Try to extrasd the t value and p-value for group:time interasdion
...
write.csv(results, file = "CAT12_results/SD/results.csv", na = "", row.names = F)
...
# Plot the interasdion of time by class
...
ggplot(class_means, aes(x = time, y = mean_ROI_sum, group = class)) +
  ...
  labs(title = "Differences in SD trajectories", x = "Age", y = "Sum of adjusted SD across ROIs") +
  theme_bw() +
  theme(
...
ggsave("CAT12_results/SD/plot_new.pdf", width=4, height=4, dpi=300)
